...
class AppStore extends Component {
  state = {tabCategory: tabsList[0].tabId, searchInput: ''}
  ...
  render() {
    const {tabCategory, searchInput} = this.state
    //  console.log(tabCategory)
    //  console.log(searchInput)
    //  filter by category
    const filteredAppsList = appsList.filter(
      eachItem => eachItem.category === tabCategory,
    )
    //  filers by searchInput
    let updatedAppsList
    switch (searchInput) {
      case '':
        updatedAppsList = filteredAppsList.filter(eachItem =>
          eachItem.category.toLowerCase().includes(searchInput.toLowerCase()),
        )
        break
